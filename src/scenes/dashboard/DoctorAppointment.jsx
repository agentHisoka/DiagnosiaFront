...
const DoctorAppointment = () => {
  ...
  const renderAppointment = (date, hour) => {
    const appointment = appointments.find(
      (apt) => apt.date.toDateString() === date.toDateString() && apt.hour === hour
    );
      ...
      return (
        <div className="bg-white p-2 rounded shadow transform hover:scale-105 transition-transform duration-300 text-gray-500">
          <div className="flex items-center">
            ...
            <p className="text-blue-500 font-semibold">{appointment.patientName}</p>
          </div>
  ...
  return (
    <div className="container mx-auto mt-5">
      <h1 className="text-3xl font-semibold mb-3">Doctor's Appointment Calendar</h1>
      <div className="grid grid-cols-8 gap-2">
        ...
        {hours.map((hour) => (
          <React.Fragment key={hour}>
            <div className="col-span-1 text-right pr-2 py-1">
              {hour}
            </div>
            {dates.map((date) => (
